...
import com.shounakmulay.telephony.utils.Constants.SIMSLOT
...
class SmsMethodCallHandler(
  ...
  private lateinit var simSlot: String
    ...
    when (action.toActionType()) {
      ...
      ActionType.SEND_SMS -> {
        if (call.hasArgument(MESSAGE_BODY)
          ...
          val simSlot = call.argument<String>(SIMSLOT)
  ...
  private fun handleSendSmsActions(smsAction: SmsAction) {
    ...
    when (smsAction) {
      SmsAction.SEND_SMS -> smsController.sendSms(address, messageBody, simSlot, listenStatus)
      SmsAction.SEND_MULTIPART_SMS -> smsController.sendMultipartSms(address, messageBody,simSlot, listenStatus)
